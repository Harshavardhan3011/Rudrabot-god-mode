...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    await setShieldFlag(interaction.guild.id, interaction.guild.name, interaction.guild.ownerId, 'antiVanitySteal', enabled);

    const embed = new EmbedBuilder()
      .setColor(enabled ? '#22C55E' : '#EF4444')
      .setTitle('Shield Anti-Vanity-Steal')
      .setDescription(enabled ? 'Vanity URL protection is enabled.' : 'Vanity URL protection is disabled.')
      .addFields(
        { name: 'Flag', value: 'antiVanitySteal', inline: true },
